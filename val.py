...
def run(
    ...
    if not training:
        ...
        task = task if task in ("train", "val", "test", "golden_search") else "val"  # path to train/val/test images
    ...
    if task == "golden_search":
        LOGGER.info("Starting Golden Section Search for optimal conf_thres...")
        original_task = task  # 保存原始任务类型
        original_verbose = verbose  # 保存原始verbose设置

        # 定义评估函数（内部封装run调用）
        def evaluate(conf):
            nonlocal model, dataloader  # 复用已加载的模型和数据
            results, _, _ = run(
                data=data,
                model=model,
            ...
            return results[2]  # 返回mAP50

        # 黄金分割参数配置
        golden_ratio = (math.sqrt(5) - 1) / 2  # ≈ 0.618
        low, high = 0.001, 0.5  # 初始搜索区间
        tol = 1e-2  # 收敛阈值
        max_iter = 10  # 最大迭代次数
        best_map = 0.0
        optimal_conf = conf_thres

        # 预加载模型和数据（确保只加载一次）
        if model is None:
            model = DetectMultiBackend(weights, device=device, dnn=dnn, data=data, fp16=half)
        if dataloader is None:
            dataloader = create_dataloader(...)[0]  # 使用原有参数创建dataloader
        ...
        for _ in range(max_iter):
            ...
            LOGGER.info(f"Iter {_ + 1}: conf_thres=[{low:.4f}, {high:.4f}], best_mAP={best_map:.4f}")

        # 用最优值执行最终验证
        LOGGER.info(f"\nOptimal conf_thres: {optimal_conf:.4f} (mAP50-95: {best_map:.4f})")
        return run(
            data=data,
            model=model,
            dataloader=dataloader,
            task=original_task.replace("golden_search", "val"),  # 恢复原始任务
            conf_thres=optimal_conf,
            verbose=original_verbose,  # 恢复原始verbose设置
            # 恢复其他原始参数
            save_txt=save_txt,
            save_json=save_json,
            plots=True,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Inference
        with dt[1]:
            (preds, train_out), _ = model(im) if compute_loss else (model(im, augment=augment), None)
...
def parse_opt():
    ...
    parser.add_argument("--weights", nargs="+", type=str,
                        default=r"D:\Writing\yolov5\runs\train\exp14\weights\best.pt",
                        help="model path(s)")
    ...
    parser.add_argument("--task", default="golden_search", help="train, val, test, speed or study,golden_search")
...
def main(opt):
    ...
    if opt.task in ("train", "val", "test", "golden_search"):  # run normally
