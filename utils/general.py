...
def remove_high_iou_across_classes(output, iou_thres=0.9):
    """
    在不同类别间检查IoU并移除IoU超过阈值的低置信度框。

    Args:
        output: NMS之后的结果，每个图像上的检测结果列表，每个元素是一个(n, 6+nm)张量，表示[xyxy, conf, cls]。
        iou_thres (float): IoU阈值，用于决定是否删除重叠框，默认是0.9。

    Returns:
        list of detections after removing high IoU boxes across different classes.
    """
    for xi, x in enumerate(output):
        ...
        # 计算所有预测框之间的IoU矩阵
        ious = box_iou(x[:, :4], x[:, :4])

        # 设置对角线为0，因为我们不关心同一个框与自己的IoU
        ious.fill_diagonal_(0)

        # 找出IoU大于阈值的框对
        high_iou_indices = torch.where(ious > iou_thres)

        # 如果没有找到任何高IoU的框对，则继续下一个图像
        if high_iou_indices[0].shape[0] == 0:
            continue

        # 删除置信度较低的框
        for i, j in zip(high_iou_indices[0], high_iou_indices[1]):
            if x[i, 4] < x[j, 4]:
                x[i, 4] = 0  # 将置信度设置为0，表示忽略此框
            else:
                x[j, 4] = 0
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # x[:, 5:] = x[:, 4:5] * x[:, 5:] * (1 - torch.std(x[:, 5:]))
        # conf_thres1= conf_thres + 0.1 * torch.mean(x[:, 5:]).item()
        # print(conf_thres1)
