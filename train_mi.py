...
from utils.loss import ComputeLoss, ComputeLossLD
...
#     dir_list = [r"D:\Writing\datasets\Fog\labels_teacher\train_norm",
#                 r"D:\Writing\datasets\Fog\labels_teacher\train_fog"]
#     teacher_labels = []
#     for dir in dir_list:
#         txts_path = os.listdir(dir)
#         for txt_path in txts_path:
#             abs_path = os.path.join(dir, txt_path)
#             contents = read_txt(abs_path)
#             teacher_labels.append(contents)
#     return teacher_labels

# def compute_iou(box1, box2):
#     """
#     Compute IoU between two sets of bounding boxes.
#     box1 and box2 formats: [x, y, w, h].
#     """
#     # Convert to [x1, y1, x2, y2] format
#     box1 = torch.stack([box1[:, 0] - box1[:, 2] / 2, box1[:, 1] - box1[:, 3] / 2,
#                         box1[:, 0] + box1[:, 2] / 2, box1[:, 1] + box1[:, 3] / 2], dim=1)
#     box2 = torch.stack([box2[:, 0] - box2[:, 2] / 2, box2[:, 1] - box2[:, 3] / 2,
#                         box2[:, 0] + box2[:, 2] / 2, box2[:, 1] + box2[:, 3] / 2], dim=1)
#
...
#
#
# def match_instances(targets, teacher_targets, iou_threshold=0.5):
#     target_classes = targets[:, 1:2]
#     teacher_classes = teacher_targets[:, 1:2]
#     target_boxes = targets[:, 2:]
#     teacher_boxes = teacher_targets[:, 2:]
#     # teacher_weights=teacher_targets[:, -1]
#
#     iou_matrix = compute_iou(target_boxes, teacher_boxes)
#     mask_cls = target_classes == teacher_classes.T
#     mask_box = iou_matrix >= iou_threshold
#     mask = mask_cls & mask_box
#     matched_indices = torch.nonzero(mask)
#
#     # matched_teacher = teacher_targets[matched_indices[:, 1]] if len(matched_indices) > 0 else torch.tensor([])
#
#     unmatched_targets = [teacher_targets[i] for i in range(len(teacher_targets)) if i not in matched_indices[:, 1]]
#     unmatched_targets = torch.stack(unmatched_targets, dim=0) if unmatched_targets else torch.tensor([])
#
#     # final_targets = torch.cat([matched_teacher, unmatched_targets], dim=0) if len(matched_teacher) > 0 or len(
#     #     unmatched_targets) > 0 else torch.tensor([])
#
#     # num_targets = unmatched_targets.size(0)
#     # num_selected = num_targets // 2
#     # selected_indices = torch.randperm(num_targets)[:num_selected]
#     #
#     # selected_targets = unmatched_targets[selected_indices]
#
...
def train(hyp, opt, device, callbacks):
    ...
    Example:
        ...
        - Tutorial: https://docs.ultralytics.com/yolov5/tutorials/train_custom_data
    """
    save_dir, epochs, batch_size, weights, t_weights, single_cls, evolve, data, cfg,t_cfg, resume, noval, nosave, workers, freeze = (
        Path(opt.save_dir),
        opt.epochs,
        opt.batch_size,
        opt.weights,
    ...
    compute_loss_ld = ComputeLossLD(model)  # init loss class
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            #### read teacher targets ####
            # teacher_paths = [path.replace("images", "labels_teacher") for path in paths]
            # teacher_paths = ["".join(path.split(".")[:-1]) + ".txt" for path in teacher_paths]
            # teacher_targets = read_txt(teacher_paths)
            # teacher_targets = torch.from_numpy(np.array(teacher_targets))
            # teacher_targets = torch.cat(
            #     [teacher_targets[:, 0].unsqueeze(1), teacher_targets[:, -1].unsqueeze(1), teacher_targets[:, 1:5],
            #      torch.unsqueeze(torch.max(teacher_targets[:, 5:10], dim=-1)[0], -1)],
            #     dim=1)
            # teacher_targets = torch.cat(
            #     [teacher_targets[:, 0].unsqueeze(1), teacher_targets[:, -1].unsqueeze(1), teacher_targets[:, 1:5]],
            #     dim=1)
            # targets = torch.cat([targets, torch.ones(targets.shape[0], 1)], dim=1)
            ...
            # teacher_targets = match_instances(targets, teacher_targets, 0.4)
            # Forward
            with torch.cuda.amp.autocast(amp):
                pred = model(imgs)  # forward
                loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
                # loss, loss_items = compute_loss_ld(pred, teacher_targets.to(device))  # loss scaled by batch_size
                # loss2, loss_items = compute_loss(pred, teacher_targets.to(device))  # loss scaled by batch_size
                # loss = loss1 + loss2
                # loss1 = 0
                # loss2 = 0
                # # if len(targets_matched) > 0:
                # #     loss1, loss_items = compute_loss(pred, targets_matched.to(device))
                # if len(targets_unmatched) > 0:
                #     loss2, loss_items = compute_loss(pred, targets_unmatched.to(device))
...
def parse_opt(known=False):
    ...
    parser.add_argument("--t_weights", type=str, default=ROOT / "yolov5s.pt", help="initial weights path")
    parser.add_argument("--t_cfg", type=str, default="yolov5s.yaml", help="model.yaml path")

    parser.add_argument("--weights", type=str, default=ROOT / "yolov5s.pt", help="initial weights path")
    parser.add_argument("--cfg", type=str, default="yolov5s.yaml", help="model.yaml path")
    parser.add_argument("--data", type=str, default=ROOT / "data/fog.yaml", help="dataset.yaml path")
