...
class MEA_Block(nn.Module):
    def __init__(self, channel, length, b=1, gamma=2):
        super(MEA_Block, self).__init__()
        kernel_size = int(abs((math.log(channel, 2) + b) / gamma))
        kernel_size = kernel_size if kernel_size % 2 else kernel_size + 1

        spatial_size = int(abs((math.log(length, 2) + b) / gamma))
        spatial_size = spatial_size if spatial_size % 2 else spatial_size + 1

        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv0 = nn.Conv1d(1, 1, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, bias=False)
        self.conv1 = nn.Conv1d(1, 1, kernel_size=spatial_size, padding=(spatial_size - 1) // 2, bias=False)
        self.conv2 = nn.Conv1d(1, 1, kernel_size=spatial_size, padding=(spatial_size - 1) // 2, bias=False)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        y0 = self.avg_pool(x)
        y0 = self.conv0(y0.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
        y0 = self.sigmoid(y0)
        out0 = x * y0.expand_as(x)
        y1 = torch.transpose(x, 1, 2)
        y1 = self.avg_pool(y1)
        y1 = self.conv1(y1.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
        y1 = self.sigmoid(y1)
        y1 = torch.transpose(y1, 1, 2)
        y2 = torch.transpose(x, 1, 3)
        y2 = self.avg_pool(y2)
        y2 = self.conv2(y2.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
        y2 = self.sigmoid(y2)
        y2 = torch.transpose(y2, 1, 3)
        y3 = y1 * y2
        out3 = x * y3.expand_as(x)
        out = (out0 + out3) / 2.0
        return out
...
class BaseModel(nn.Module):
    ...
    def __init__(self):
        ...
        # self.moe = ViTMoE(dim=512)
        # self.num_experts = 5
        # self.gate = MoEGate(512, self.num_experts, 1)
        ...
        # self.cbam1 = CBAM(128, 16, 3)
        # self.cbam2 = CBAM(128, 16, 3)
        # self.cbam3 = CBAM(128, 16, 3)
        # self.cbam4 = CBAM(128, 16, 3)
        # self.cbam5 = CBAM(128, 16, 3)
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            #     expert_outputs = torch.stack([eca(x) for eca in self.ecas], dim=1)
            #
            #     # 使用权重融合专家输出
